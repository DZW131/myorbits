	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRun {
				...
				if jsonOutput {
					return emitJSON(cmd.OutOrStdout(), templateSavePreviewPayload(resolved.Repo.Root, preview))
				}
			...
			result, err := harnesspkg.SaveTemplateBranch(cmd.Context(), harnesspkg.TemplateSaveInput{
