...
func NewTemplateApplyCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "apply <template-branch|git-url>",
		...
		Long: "Legacy compatibility wrapper around `harness install`.\n" +
			...
			"Use a local template branch name directly, or pass a remote Git URL. --ref is only valid for remote Git URLs.",
		Example: "" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if localSource {
				if strings.TrimSpace(requestedRef) != "" {
					return fmt.Errorf("--ref is only supported when applying from a remote Git URL")
				}
	...
	cmd.Flags().String("ref", "", "Select one remote template branch explicitly when applying from a remote Git URL")
...
func emitTemplateApplyPreview(cmd *cobra.Command, preview orbittemplate.TemplateApplyPreview, jsonOutput bool, overwriteExisting bool) error {
	...
	if len(preview.ResolvedBindings) == 0 {
		...
		for _, name := range names {
			...
			if _, err := fmt.Fprintf(cmd.OutOrStdout(), "%s <- %s\n", name, stringifyBindingSource(resolved.Source)); err != nil {
				return fmt.Errorf("write command output: %w", err)
			}
		}
	}
...
func templateApplyBindingsPayload(resolvedBindings map[string]bindings.ResolvedBinding) []templateApplyBindingJSON {
	...
	for _, name := range names {
		items = append(items, templateApplyBindingJSON{
			Name:   name,
			Source: stringifyBindingSource(resolvedBindings[name].Source),
		})
	}
