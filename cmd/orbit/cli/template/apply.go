...
func loadTemplateSourceAtRevision(ctx context.Context, repoRoot string, revision string, sourceRef string, sourceCommit string) (LocalTemplateSource, error) {
	...
	definitionPath, _, spec, err := loadTemplateCompanionAtRevision(ctx, repoRoot, revision, manifest.Template.OrbitID)
	if err != nil {
		return LocalTemplateSource{}, err
	}
...
func BuildTemplateApplyPreview(ctx context.Context, input TemplateApplyPreviewInput) (TemplateApplyPreview, error) {
	...
	return buildTemplateApplyPreviewFromSource(ctx, input.RepoRoot, source, Source{
		SourceKind:     InstallSourceKindLocalBranch,
		SourceRepo:     "",
		SourceRef:      source.Ref,
		TemplateCommit: source.Commit,
	}, input.BindingsFilePath, input.Interactive, input.Prompter, input.EditorMode, input.Editor, input.Now)
}
...
func BuildRemoteTemplateApplyPreview(ctx context.Context, input RemoteTemplateApplyPreviewInput) (TemplateApplyPreview, error) {
	...
	preview, err := buildTemplateApplyPreviewFromSource(ctx, input.RepoRoot, source, Source{
...
func buildTemplateApplyPreviewFromSource(
	...
	if err != nil {
		return TemplateApplyPreview{}, fmt.Errorf("load --bindings file: %w", err)
	}
	repoVarsFile, hasRepoVarsFile, err := loadOptionalRepoVarsFile(ctx, repoRoot)
	if err != nil {
		return TemplateApplyPreview{}, fmt.Errorf("load runtime vars: %w", err)
	}
	...
	mergeResult, err := resolveTemplateBindings(ctx, declared, bindingsFile, repoVars, interactive, prompter, editorMode, editor)
	if err != nil {
	...
	if len(mergeResult.Unresolved) > 0 {
		...
		return TemplateApplyPreview{}, fmt.Errorf("missing required bindings: %s", strings.Join(names, ", "))
	}
	...
	renderedFiles, err := RenderTemplateFiles(source.Files, renderValues)
	if err != nil {
		return TemplateApplyPreview{}, fmt.Errorf("render template files: %w", err)
	}
	renderedSharedAgentsFile, hasSharedAgents, err := renderSharedAgentsPayload(source, renderValues)
	if err != nil {
...
func ApplyLocalTemplate(ctx context.Context, input TemplateApplyInput) (TemplateApplyResult, error) {
	...
	return applyTemplatePreview(input.Preview.RepoRoot, preview, input.Preview.OverwriteExisting)
}
...
func applyTemplatePreview(repoRoot string, preview TemplateApplyPreview, overwriteExisting bool) (TemplateApplyResult, error) {
...
func loadOptionalBindingsFile(filename string) (map[string]bindings.VariableBinding, error) {
...
func analyzeApplyConflicts(
	...
	if renderedSharedAgentsFile != nil {
