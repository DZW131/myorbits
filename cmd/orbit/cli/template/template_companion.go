...
func loadTemplateCompanionAtRevision(
	...
	revision string,
	orbitID string,
) (string, []byte, orbitpkg.OrbitSpec, error) {
	hostedPath, legacyPath, err := templateCompanionPaths(orbitID)
	if err != nil {
		return "", nil, orbitpkg.OrbitSpec{}, err
	}

	hostedExists, err := gitpkg.PathExistsAtRev(ctx, repoRoot, revision, hostedPath)
	if err != nil {
		return "", nil, orbitpkg.OrbitSpec{}, fmt.Errorf("check template definition %s from %q: %w", hostedPath, revision, err)
	}
	if hostedExists {
		data, err := gitpkg.ReadFileAtRev(ctx, repoRoot, revision, hostedPath)
		if err != nil {
			return "", nil, orbitpkg.OrbitSpec{}, fmt.Errorf("read template definition %s from %q: %w", hostedPath, revision, err)
		}
		spec, err := orbitpkg.ParseHostedOrbitSpecData(data, hostedPath)
		if err != nil {
			return "", nil, orbitpkg.OrbitSpec{}, fmt.Errorf("parse template definition %s from %q: %w", hostedPath, revision, err)
		}

		return hostedPath, data, spec, nil
	}

	data, err := gitpkg.ReadFileAtRev(ctx, repoRoot, revision, legacyPath)
	if err != nil {
		return "", nil, orbitpkg.OrbitSpec{}, fmt.Errorf("read template definition %s from %q: %w", legacyPath, revision, err)
	}
	spec, err := orbitpkg.ParseOrbitSpecData(data, legacyPath)
	if err != nil {
		return "", nil, orbitpkg.OrbitSpec{}, fmt.Errorf("parse template definition %s from %q: %w", legacyPath, revision, err)
	}

	return legacyPath, data, spec, nil
}
