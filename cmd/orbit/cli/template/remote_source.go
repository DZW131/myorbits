...
func (err *RemoteTemplateNotFoundError) Error() string {
	...
	case RemoteTemplateNotFoundReasonSourceAliasMissingPublishOrbitID:
		return fmt.Sprintf(
			"remote source branch ref %q from %q is missing source.orbit_id; update .harness/manifest.yaml before installing from the source branch",
			err.RequestedRef,
			err.RepoURL,
		)
	case RemoteTemplateNotFoundReasonSourceAliasPublishedTemplateMissing:
		return fmt.Sprintf(
			"remote source branch ref %q from %q resolves to %q, but that published template branch does not exist; run `orbit template publish` first",
			err.RequestedRef,
			err.RepoURL,
			err.ResolvedRef,
		)
	case RemoteTemplateNotFoundReasonSourceAliasPublishedTemplateInvalid:
		return fmt.Sprintf(
			"remote source branch ref %q from %q resolves to %q, but that branch is not a valid orbit template branch",
			err.RequestedRef,
			err.RepoURL,
			err.ResolvedRef,
		)
	...
	if strings.TrimSpace(err.RequestedRef) != "" {
		if err.SourceBranch {
			return fmt.Sprintf(
				"remote template ref %q from %q is a source branch, not a valid template branch; run `orbit template publish` first",
				err.RequestedRef,
				err.RepoURL,
			)
		}
		return fmt.Sprintf("remote template ref %q from %q is not a valid template branch", err.RequestedRef, err.RepoURL)
	}

	return fmt.Sprintf("no valid remote template branches found in %q", err.RepoURL)
}
...
func (err *RemoteTemplateAmbiguityError) Error() string {
	return fmt.Sprintf("remote template source %q is ambiguous; candidates: %s", err.RepoURL, strings.Join(err.BranchNames(), ", "))
}
...
func ResolveRemoteTemplateCandidateSnapshot(ctx context.Context, repoRoot string, candidate RemoteTemplateCandidate) (LocalTemplateSource, error) {
	...
	if err := gitpkg.WithFetchedRemoteRef(ctx, repoRoot, candidate.RepoURL, candidate.Ref, func(tempRef string) error {
		...
		return LocalTemplateSource{}, fmt.Errorf("resolve remote template source %q from %q: %w", candidate.Branch, candidate.RepoURL, err)
	}
...
func resolveExplicitRemoteTemplateSourceSnapshot(
	...
	if err := gitpkg.WithFetchedRemoteRef(ctx, repoRoot, remoteURL, requestedRef, func(tempRef string) error {
		...
		return RemoteTemplateCandidate{}, LocalTemplateSource{}, fmt.Errorf(
			"resolve remote template source %q from %q: %w",
			requestedRef,
			remoteURL,
			err,
		)
