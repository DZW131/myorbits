...
func resolveInstalledTemplateSource(ctx context.Context, repoRoot string, record InstallRecord) (LocalTemplateSource, error) {
	...
	case InstallSourceKindRemoteGit:
		...
		if err := gitpkg.WithFetchedRemoteRef(ctx, repoRoot, record.Template.SourceRepo, remoteRef, func(_ string) error {
			...
			return LocalTemplateSource{}, fmt.Errorf("resolve recorded remote template source %q from %q: %w", record.Template.SourceRef, record.Template.SourceRepo, err)
		}
