...
func MaterializeOrbitBrief(ctx context.Context, input BriefMaterializeInput) (BriefMaterializeResult, error) {
	...
	if err := ensureBriefMaterializeRevisionAllowed(input.RepoRoot); err != nil {
	...
	payload, hasTruth, err := materializedOrbitBriefPayload(spec, input.RepoRoot)
	if err != nil {
		return BriefMaterializeResult{}, err
	}
	if !hasTruth {
...
func materializedOrbitBriefPayload(spec orbitpkg.OrbitSpec, repoRoot string) ([]byte, bool, error) {
	...
	renderedFiles, err := RenderTemplateFiles([]CandidateFile{{
		Path:    runtimeAgentsRepoPath,
		Content: body,
	}}, renderBindingValues(runtimeBindings))
	if err != nil {
