...
func renderSharedAgentsPayload(source LocalTemplateSource, bindings map[string]string) (CandidateFile, bool, error) {
...
func removeRuntimeAgentsBlock(repoRoot string, orbitID string) error {
	...
	updated, removed, err := removeRuntimeAgentsBlockFromData(existing, orbitID)
	if err != nil {
...
func removeRuntimeAgentsBlockFromData(existing []byte, orbitID string) ([]byte, bool, error) {
	if _, err := ParseRuntimeAgentsDocument(existing); err != nil {
		return nil, false, fmt.Errorf("parse runtime AGENTS.md: %w", err)
	}
...
func RemoveRuntimeAgentsBlockData(existing []byte, blockID string) ([]byte, bool, error) {
	return removeRuntimeAgentsBlockFromData(existing, blockID)
}
