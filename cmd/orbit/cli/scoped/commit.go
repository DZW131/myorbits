...
func Commit(
	...
	if err := viewpkg.WriteWarnings(store, current.Orbit, result.WarningMessages); err != nil {
		return result, fmt.Errorf("write warning snapshot: %w", err)
	}
	...
	statusEntries, err = gitpkg.WorktreeStatus(ctx, repo.Root)
	if err != nil {
		return result, fmt.Errorf("load worktree status after orbit commit: %w", err)
	}
	if err := viewpkg.WriteActiveRuntimeAndGitLedger(
		...
		return result, fmt.Errorf("write runtime and git ledger after orbit commit: %w", err)
	}
