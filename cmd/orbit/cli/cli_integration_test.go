...
func TestOrbitCommandWorkflow(t *testing.T) {
	...
	require.Contains(t, stdout, "include:\n")
...
func TestAddJSONOutputStable(t *testing.T) {
	...
	require.Equal(t, "docs", output.Definition.ID)
	require.Equal(t, "docs orbit", output.Definition.Description)
	require.Equal(t, []string{"docs/**"}, output.Definition.Include)
	require.Empty(t, output.Definition.Exclude)
	require.Empty(t, output.Definition.SourcePath)

	raw := decodeJSONMap(t, stdout)
	requireJSONKeys(t, raw, "definition", "file", "repo_root")
	definitionRaw := requireNestedJSONMap(t, raw, "definition")
	requireJSONKeys(t, definitionRaw, "Description", "Exclude", "ID", "Include", "SourcePath")
}
...
func TestAddSupportsMemberSchemaFlag(t *testing.T) {
	...
	require.Contains(t, string(definitionData), "members:\n")
	require.Contains(t, string(definitionData), "role: subject\n")
	require.Contains(t, string(definitionData), "rules:\n")
}
...
func TestShowJSONOutputStable(t *testing.T) {
	...
	require.Equal(t, []string{"docs/**"}, output.Orbit.Include)
	require.Empty(t, output.Orbit.Exclude)
	...
	requireJSONKeys(t, orbitRaw, "Description", "Exclude", "ID", "Include", "SourcePath")
}
...
func TestShowDisplaysMemberSchemaView(t *testing.T) {
	...
	require.Contains(t, stdout, "key: docs-process\n")
	require.Contains(t, stdout, "role_scopes:\n")
...
func TestShowJSONOutputIncludesMemberSchemaView(t *testing.T) {
	...
	requireJSONKeys(t, raw, "orbit", "repo_root")
	orbitRaw := requireNestedJSONMap(t, raw, "orbit")
	requireJSONKeys(t, orbitRaw, "description", "id", "members", "meta", "role_scopes", "schema", "source_path")
