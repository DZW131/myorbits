...
func defaultMemberSchemaSpec(
	...
	spec := OrbitSpec{
		...
		Rules: &OrbitRules{
			...
			Orchestration: OrbitOrchestrationRules{
...
func WriteOrbitSpec(repoRoot string, spec OrbitSpec) (string, error) {
	if err := ValidateOrbitSpec(spec); err != nil {
		return "", fmt.Errorf("validate orbit spec: %w", err)
	}

	return writeOrbitSpecAtPath(repoRoot, spec, DefinitionPath)
}
...
func WriteHostedOrbitSpec(repoRoot string, spec OrbitSpec) (string, error) {
	...
	return writeOrbitSpecAtPath(repoRoot, spec, HostedDefinitionPath)
}
