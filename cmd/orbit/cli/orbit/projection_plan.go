...
func ResolveProjectionPlan(config RepositoryConfig, spec OrbitSpec, trackedFiles []string) (ProjectionPlan, error) {
	...
	for _, trackedFile := range trackedFiles {
		...
		for index, member := range spec.Members {
			...
			if matchErr != nil {
				return ProjectionPlan{}, fmt.Errorf("match member %q for %q: %w", member.Key, normalizedPath, matchErr)
			}
			if !matches {
				continue
			}
...
func defaultProjectionPlanRoleScopes(spec OrbitSpec) roleScopeDefaults {
	...
	if spec.Rules.Scope.ProjectionRoles != nil {
		defaults.setProjectionRoles(spec.Rules.Scope.ProjectionRoles)
	}
	if spec.Rules.Scope.WriteRoles != nil {
		defaults.setWriteRoles(spec.Rules.Scope.WriteRoles)
	}
	if spec.Rules.Scope.ExportRoles != nil {
		defaults.setExportRoles(spec.Rules.Scope.ExportRoles)
	}
	if spec.Rules.Scope.OrchestrationRoles != nil {
		defaults.setOrchestrationRoles(spec.Rules.Scope.OrchestrationRoles)
	}
