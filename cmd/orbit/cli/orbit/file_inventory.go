...
func BuildFileInventorySnapshot(
	...
	paths := mergeSortedUniquePaths(plan.MetaPaths, plan.SubjectPaths, plan.RulePaths, plan.ProcessPaths)
	...
	for _, path := range paths {
		...
		memberKey, err := fileInventoryMemberKey(spec, path, classification.Role)
		if err != nil {
			return statepkg.FileInventorySnapshot{}, fmt.Errorf("resolve member key for %q: %w", path, err)
		}
...
func fileInventoryMemberKey(spec OrbitSpec, normalizedPath string, role string) (string, error) {
	if role == PathRoleMeta || !spec.HasMemberSchema() {
		return "", nil
	}
	...
	for _, member := range spec.Members {
		...
		if err != nil {
			return "", fmt.Errorf("match member %q: %w", member.Key, err)
		}
		if !matches {
			continue
		}
